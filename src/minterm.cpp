...
uint32_t minterm::get(size_t uid)
{
	if (uid >= size)
...
uint32_t minterm::val(size_t uid)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::set(size_t uid, uint32_t v)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::resize(size_t s, uint32_t r)
{
	s--;
	size_t w, l;
	if (s >= size)
...
void minterm::sv_union(size_t uid, uint32_t v)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::sv_intersect(size_t uid, uint32_t v)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::sv_invert(size_t uid)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::sv_or(size_t uid, uint32_t v)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::sv_and(size_t uid, uint32_t v)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
void minterm::sv_not(size_t uid)
{
	if (uid >= size)
		resize(uid+1, default_value);
...
vector<minterm> minterm::expand(vector<int> uids)
{
	...
	{
		for (size_t j = r1.size()-1; j >= 0 && j < r1.size(); j--)
		{
...
minterm::minterm(tokenizer &tokens, string exp, variable_space &vars)
{
	default_value = vX;
	string var;
	uint32_t value;
...
void minterm::vars(vector<size_t> *var_list)
{
	for (size_t i = 0; i < size; i++)
		if (get(i) != vX)
...
minterm minterm::refactor(vector<size_t> ids)
{
	minterm result;
...
minterm minterm::refactor(vector<pair<size_t, size_t> > ids)
{
	minterm result;
	for (size_t i = 0; i < ids.size(); i++)
...
minterm minterm::hide(size_t var)
{
	minterm result = *this;
	if (var < size)
...
minterm minterm::hide(vector<size_t> vars)
{
	minterm result = *this;
...
string minterm::print(variable_space &vars, string prefix)
{
	...
	for (size_t i = 0; i < values.size(); i++)
	{
		...
		for (int j = 15; j > 0; j--)
		{
			if (((v>>(2*j))&3) != 3 && (i*16 + (15-j)) < size)
			{
				if (first)
					res += "&";

				res += tbl[(v>>(2*j))&3] + prefix + vars.at(i*16 + (15-j)).name;
				first = true;
			}
		}

		if ((v&3) != 3 && (i*16 + 15) < size)
		{
			if (first)
				res += "&";

			res += tbl[v&3] + prefix + vars.at(i*16 + 15).name;
			first = true;
		}
	}
...
string minterm::print_assign(variable_space &vars, string prefix)
{
	...
	for (size_t i = 0; i < values.size(); i++)
	{
		...
		for (int j = 15; j > 0; j--)
		{
			if (((v>>(2*j))&3) != 3 && ((v>>(2*j))&3) != 0 && (i*16 + (15-j)) < size)
			{
				if (first)
				{
					res += ",";
				...
				res += prefix + vars.at(i*16 + (15-j)).name;
				res2 += tbl[(v>>(2*j))&3];

				first = true;
			}
		}
		if ((v&3) != 3 && (v&3) != 0 && (i*16 + 15) < size)
		{
			if (first)
			{
				res += ",";
			...
			res += prefix + vars.at(i*16 + 15).name;
			res2 += tbl[v&3];
